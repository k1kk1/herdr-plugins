...
### Alfred は引数を argv で渡さない

Run Script の「input as argv」を設定しても、実測では `$1` が空でした
（スクリプトは `/bin/bash` として起動し、位置パラメータは1つもありません）。
渡ってくるのは **スクリプト本文の `{query}` を置換する形**だけです。

argv へのフォールバックは**意図的に書いていません。** ガードを1つ置くには
本文にもう1つ `{query}` が要りますが、**Alfred は全部の出現を置換する**ので
ガードごと書き換わり、置換が成功したときに限ってフォールバックが誤発火します。
置換が止まった場合はリテラルのトークンがそのままバイナリに届き、
**それと分かるエラー文**になります。壊れ方としてはそちらの方がましです。
