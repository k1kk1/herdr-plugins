...
開き先のターミナルは、**Herdr が実際に動いているアプリ**を優先します。
`herdr` のプロセスから親を辿って、アプリバンドルに行き当たったところがそれです。

```
herdr → -/bin/zsh → /usr/bin/login → /Applications/cmux.app/…/cmux
```

`$TERM_PROGRAM` を見ないのは、**Alfred から呼ばれたときに設定されていない**からです。
インストール済みのものから選ぶ方式も駄目で、cmux で Herdr を動かしている人の画面に
**空の Ghostty ウィンドウ**を開いてしまいました。

そのアプリにコマンドを渡す方法が分からない場合（cmux など）は、渡せる別の
ターミナルで開いたうえで、**どれを使ったかを結果に添えます。** 使っていない
アプリのウィンドウが出てくるのは驚きなので、黙って済ませません。

それ以外のターミナルは `command` を書いてください。
...
| `hr` | Claude Code / Codex の会話 | **動いている Herdr の中で再開**し、そのターミナルを前面に出す |
...
再開後にターミナルを前面に出す処理は、**すでに動いているアプリしか対象にしません。**
`open -a` は動いていないアプリを**起動する**ので、推測した相手に対して使うと、
何も入っていない新品のウィンドウが出てきます。実際にそうなりました。

対象を設定や推測ではなくプロセスツリーから取っているのはそのためです。
そこから得たアプリは、定義上すでに動いています。
...
Alfred はオブジェクトのアイコンを **uid と同名の PNG** から読むので、
インストーラが書き出しています。

### uid は毎回同じでなければならない

オブジェクトの uid をインストールごとに作り直すと、**Alfred は「更新された
ワークフロー」ではなく「別のワークフロー」として扱います。** キーワードの一覧は
出ます（スクリプトは読み直されるので）が、その裏の接続は Alfred がもう持って
いないオブジェクトを指していて、**Enter を押しても何も起きません。**
アイコンのファイル名も毎回変わって溜まっていきます。

なので uid は bundle id と役割から導出しています。ハッシュは FNV-1a です
（`DefaultHasher` は**リリース間で出力が変わらない保証がない**ので、
Rust を上げたら uid が変わってしまいます）。
