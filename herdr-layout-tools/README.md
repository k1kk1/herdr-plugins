...
メニューで項目を選ぶと、下部に**ショートカット実行後の配置**が表示されます。
強調された区画が現在の Pane です。各区画の番号と、その上の番号付き Pane 名を対応
させるため、どの Pane がどこへ並ぶかも分かります。保存済みレイアウトも、Pane 数が
合う場合は適用後の形を同じように確認できます。
