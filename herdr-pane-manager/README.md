...
各行のプレビューは、**その操作をしたあとどうなるか**を描きます。Tab をまたぐ
操作は**2枚を並べて**、矢印でつなぎます。1枚では「どこから出るか」か
「どこへ入るか」の片方しか描けないためです。
...
| 操作 | 左 | 右 |
|---|---|---|
| Move to… | この Tab（Pane が抜けた後） | 移動先に届いた姿 |
...
以前は Move / Swap / Extract が**同じ絵**でした。3つとも「今のこの Tab」を
描いていたので、操作を選び分ける役に立っていませんでした。

## 先の画面から戻れます

`Move to…` を開いて Tab 一覧を見てから気が変わったら、`Esc` で**メニューに
戻ります**。オーバーレイごと閉じるのは、メニュー自体で `Esc` を押したときだけ
です。戻る先が無いのはそこだけなので。
